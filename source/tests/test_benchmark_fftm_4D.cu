...
)
{
    ...
    std::tie( p1, p2, p3 ) = fftm::test::detail::choose_grid_4d(
        base_options,
        strategy,
        comm_info.num_procs
    );
    ...
    original.init( std::get<0>( in_sizes ), std::get<1>( in_sizes ), std::get<2>( in_sizes ), std::get<3>( in_sizes ) );
    ...
    {
        log.info_f(
            "benchmark=fftm-4d, strategy=%s, mode=%s, grid=(%zu,%zu,%zu), Nx=%zu, Ny=%zu, Nz=%zu, Nw=%zu, times=%d: avg_wall_ms=%.8e, stddev_wall_ms=%.8e",
            fftm_t::strategy_name_4d(),
            fftm::mpi_transpose_3d_mode_name( fftm_t::transpose_mode_4d ),
            p1,
            p2,
        ...
        row
            << fftm::test::detail::csv_quote( "fftm-4d" ) << ','
            << comm_info.num_procs << ','
