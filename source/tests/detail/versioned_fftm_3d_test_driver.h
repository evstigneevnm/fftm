#ifndef __FFTM_TESTS_DETAIL_VERSIONED_FFTM_3D_TEST_DRIVER_H__
#define __FFTM_TESTS_DETAIL_VERSIONED_FFTM_3D_TEST_DRIVER_H__

#ifndef FFTM_EGGER_TESTCASE
#error "FFTM_EGGER_TESTCASE must be defined before including versioned_fftm_3d_test_driver.h"
#endif

#ifndef FFTM_VERSIONED_TEST_BINARY
#error "FFTM_VERSIONED_TEST_BINARY must be defined before including versioned_fftm_3d_test_driver.h"
#endif

#ifndef FFTM_TEST_ENV
#error "FFTM_TEST_ENV must be defined before including versioned_fftm_3d_test_driver.h"
#endif
...
#include "poisson_3d_fft_common.h"
...
using options_t     = fftm::test::detail::fftm_3d_test_options;
...
struct fill_random_complex_functor
{
    ...
    __DEVICE_TAG__ void operator()( const idx_t &idx ) const
    {
        ...
        array( idx ).y = unit_random_from_state<T>( state ^ 0x94D049BB133111EBull );
    }
};
...
struct fill_periodic_poisson_3d_problem_functor
{
    ...
    __DEVICE_TAG__ void operator()( const idx_t &idx ) const
    {
        ...
        periodic_poisson_3d_problem<T>::evaluate(
...
int run_forward_random_benchmark(
    log_mpi_t &log, const options_t &options, const mpi_comm_t &comm_info
)
{
    using fftm_t       = fftm::fftm<base_fft_t, mpi_comm_t, backend_t, Strategy, log_mpi_t>;
    ...
    fftm::test::detail::log_tracked_memory_with_external_mpi(
        log, comm_info, distributed_fft, "test=fftm_v0_3d",
...
)
{
    ...
    for_each(
        fill_input_functor<ref_real_t>{ ref_in, hx, hy, hz, T( 0 ), T( 0 ), T( 0 ) }, make_range( ref_in )
    );
    for_each.wait();
    ...
    if ( distributed_fft.is_memory_profiling_enabled() || reference_fft.is_memory_profiling_enabled() )
    {
        ...
        fftm::test::detail::log_tracked_memory_categories_mpi<fftm_t>(
            log, comm_info, "test=fftm_v1_3d", internal_device_current, internal_device_peak, internal_host_current,
...
)
{
    ...
    const auto stats = compute_timing_statistics( wall_times );
    fftm::test::detail::log_tracked_memory_with_external_mpi(
        log, comm_info, distributed_fft, "test=fftm_v2_3d",
        static_cast<typename fftm_t::memory_profile_bytes_t>( fftm::test::detail::sum_bytes(
...
)
{
    ...
    const auto stats = compute_timing_statistics( wall_times );
    fftm::test::detail::log_tracked_memory_with_external_mpi(
        log, comm_info, distributed_fft, "test=fftm_v3_3d",
        static_cast<typename fftm_t::memory_profile_bytes_t>( fftm::test::detail::sum_bytes(
            fftm::test::detail::array_bytes( work ), fftm::test::detail::array_bytes( hat ) ) ),
        static_cast<typename fftm_t::memory_profile_bytes_t>( fftm::test::detail::sum_bytes(
            fftm::test::detail::array_bytes( work ), fftm::test::detail::array_bytes( hat ) ) )
...
)
{
    ...
    for_each(
        fill_periodic_poisson_3d_problem_functor<real_array_t>{
    ...
    const auto stats     = compute_timing_statistics( wall_times );
    fftm::test::detail::log_tracked_memory_with_external_mpi(
        log, comm_info, distributed_fft, "test=fftm_v4_3d",
        static_cast<typename fftm_t::memory_profile_bytes_t>( fftm::test::detail::sum_bytes(
            ...
            fftm::test::detail::array_bytes( exact_dz ), fftm::test::detail::array_bytes( numerical_solution ),
            fftm::test::detail::array_bytes( numerical_dx ), fftm::test::detail::array_bytes( numerical_dy ),
            fftm::test::detail::array_bytes( numerical_dz ), fftm::test::detail::array_bytes( solution_error_sq ),
...
template <class Strategy>
int run_case(
    std::integral_constant<int, 0>, log_mpi_t &log, const options_t &options, const mpi_comm_t &comm_info
)
{
    return run_forward_random_benchmark<Strategy>( log, options, comm_info );
}

template <class Strategy>
int run_case(
    std::integral_constant<int, 1>, log_mpi_t &log, const options_t &options, const mpi_comm_t &comm_info
)
{
    return run_forward_reference_compare<Strategy>( log, options, comm_info );
}

template <class Strategy>
int run_case(
    std::integral_constant<int, 2>, log_mpi_t &log, const options_t &options, const mpi_comm_t &comm_info
)
{
    return run_backward_random_benchmark<Strategy>( log, options, comm_info );
}

template <class Strategy>
int run_case(
    std::integral_constant<int, 3>, log_mpi_t &log, const options_t &options, const mpi_comm_t &comm_info
)
{
    return run_roundtrip_random_test<Strategy>( log, options, comm_info );
}

template <class Strategy>
int run_case(
    std::integral_constant<int, 4>, log_mpi_t &log, const options_t &options, const mpi_comm_t &comm_info
)
{
    return run_periodic_laplacian_test<Strategy>( log, options, comm_info );
}
...
)
{
    ...
    case strategy_kind::slab_pencil:
        return run_case<fftm::strategy_3d_slab_pencil<Mode>>(
            std::integral_constant<int, FFTM_EGGER_TESTCASE>(), log, options, comm_info
        );
    case strategy_kind::pencil_slab:
        return run_case<fftm::strategy_3d_pencil_slab<Mode>>(
            std::integral_constant<int, FFTM_EGGER_TESTCASE>(), log, options, comm_info
        );
