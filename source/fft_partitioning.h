...
struct partition
{
    ...
    void debug_plot( const MPIComm &mpi_, std::tuple<int, int, int> processor_grid )
    {
        ...
        for ( int k = 0; k < mpi_.num_procs; k++ )
        {
            ...
            {
                std::cout << "myid = " << mpi_.myid << ", myid_grid = (" << myid_i << "," << myid_j << "," << myid_k
                          << "), j = " << j << ", size_x[j] = " << size_x[j] << std::endl;
            }
            for ( int j = 0; j < size_y.size(); j++ )
            {
                std::cout << "myid = " << mpi_.myid << ", myid_grid = (" << myid_i << "," << myid_j << "," << myid_k
                          << "), j = " << j << ", size_y[j] = " << size_y[j] << std::endl;
            }
            for ( int j = 0; j < size_z.size(); j++ )
            {
                std::cout << "myid = " << mpi_.myid << ", myid_grid = (" << myid_i << "," << myid_j << "," << myid_k
                          << "), j = " << j << ", size_z[j] = " << size_z[j] << std::endl;
            }
            if ( size_w.size() > 0 )
            {
                ...
                {
                    std::cout << "myid = " << mpi_.myid << ", myid_grid = (" << myid_i << "," << myid_j << "," << myid_k
                              << "), j = " << j << ", size_w[j] = " << size_w[j] << std::endl;
                }
            }
        }
        mpi_.barrier();
...
{
public:
    fft_partitioning( const MPIComm &mpi ) : mpi_( mpi )
    {
    }
    ...
    void init( const processor_grid &p_g, const global_sizes &g_s )
    {
        ...
        if ( p_grid_.prod() != mpi_.num_procs )
        {
            throw std::logic_error(
                "product of process grids (" + std::to_string( p_grid_.prod() ) + ") != number of mpi processes (" +
                std::to_string( mpi_.num_procs ) + ")."
            );
        }
        if ( ( ( g_sizes_.Nw == 0 ) && ( p_grid_.p3 != 0 ) ) || ( ( g_sizes_.Nw != 0 ) && ( p_grid_.p3 == 0 ) ) )
        {
            ...
            throw std::logic_error(
                "fft_partitioning: g_sizes_.Nw = " + std::to_string( g_sizes_.Nw ) +
                " and p_grid_.p3 = " + std::to_string( p_grid_.p3 )
            );
        }
