...
__global__ void copy3D_kernel(
    char *dst_base, size_t dst_pitch, size_t dst_y_pitch, const char *src_base, size_t src_pitch, size_t src_y_pitch,
...
inline void getPitchedPtrs(
    const cudaMemcpy3DParms &p, char *&dst_base, size_t &dst_pitch, size_t &dst_y_pitch, const char *&src_base,
    size_t &src_pitch, size_t &src_y_pitch
)
{
    // destination
    dst_base =
        (char *)p.dstPtr.ptr + p.dstPos.z * p.dstPtr.pitch * p.dstPtr.ysize + p.dstPos.y * p.dstPtr.pitch + p.dstPos.x;

    dst_pitch   = p.dstPtr.pitch;
    dst_y_pitch = p.dstPtr.pitch * p.dstPtr.ysize;

    // source
    src_base = (const char *)p.srcPtr.ptr + p.srcPos.z * p.srcPtr.pitch * p.srcPtr.ysize + p.srcPos.y * p.srcPtr.pitch +
               p.srcPos.x;
...
inline void launchMemcpy3DKernel( const cudaMemcpy3DParms &p, cudaStream_t stream )
{
    ...
    char       *dst_base;
    size_t      dst_pitch, dst_y_pitch;
    const char *src_base;
    size_t      src_pitch, src_y_pitch;

    getPitchedPtrs( p, dst_base, dst_pitch, dst_y_pitch, src_base, src_pitch, src_y_pitch );

    dim3 block( 8, 8, 8 );
    dim3 grid(
        ( width + block.x - 1 ) / block.x, ( height + block.y - 1 ) / block.y, ( depth + block.z - 1 ) / block.z
    );
    ...
    case cudaMemcpyDeviceToDevice:
        copy3D_kernel<<<grid, block, 0, stream>>>(
            dst_base, dst_pitch, dst_y_pitch, src_base, src_pitch, src_y_pitch, elem_size, width, height, depth
        );
        break;
    ...
    case cudaMemcpyDeviceToHost:
    {
        ...
        copy3D_kernel<<<grid, block, 0, stream>>>(
            tmp_dev, width * elem_size, width * height * elem_size, src_base, src_pitch, src_y_pitch, elem_size, width,
            height, depth
        );
    ...
    case cudaMemcpyHostToDevice:
    {
        ...
        copy3D_kernel<<<grid, block, 0, stream>>>(
            dst_base, dst_pitch, dst_y_pitch, tmp_dev, width * elem_size, width * height * elem_size, elem_size, width,
            height, depth
        );
    ...
    case cudaMemcpyHostToHost:
        ...
        std::memcpy( p.dstPtr.ptr, p.srcPtr.ptr, width * height * depth * elem_size );
        break;
    }
...
public:
    mpi_fft_distributor( const MPIComm &mpi, const Log &log ) : mpi_( mpi ), log_( log )
    {
    }
    ~mpi_fft_distributor()
    {
    ...
    void init( std::shared_ptr<partition_t> partition_p )
    {
        ...
        std::cout << "myid: " << mpi_.myid << ", comm1_->comm(): " << comm1_->comm()
                  << ", comm2_->comm(): " << comm2_->comm() << std::endl;
        ...
        domainsize_ = std::max(
            input_dim.size_x[myid_i] * input_dim.size_y[myid_j] * ( input_dim.size_z[0] / 2 + 1 ),
            transposed1_dim.size_x[myid_i] * transposed1_dim.size_y[0] * transposed1_dim.size_z[myid_j]
        );
        domainsize_ = std::max(
            domainsize_, transposed2_dim.size_x[0] * transposed2_dim.size_y[myid_i] * transposed2_dim.size_z[myid_j]
        );
    ...
    template <
        class ArrayIn,
        class ArrayOut> //assume that those are SCFD arrays, because we need view_t here if CUDA_AWARE is not supported
    void first_transpose( const ArrayIn &in, ArrayOut &out, bool forward )
    {
        ...
        for ( std::size_t i = 0; i < comm_order1_.size(); i++ )
        {
            ...
            std::size_t islice =
                transposed1_dim.size_x[myid_i] * input_dim.start_y[p_j] * transposed1_dim.size_z[myid_j];
            ...
            scfd::communication::detail::irecv(
                ...
                scfd::communication::detail::mpi_data_type<char>::mpi_type(), p_j, p_j, comm1_->comm(), &recv_req[p_j]
            );


            // int MPI_Isend(const void *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm, MPI_Request *request)
            scfd::communication::detail::isend(
                &in_ptr[oslice], sizeof( Tin ) * osize, scfd::communication::detail::mpi_data_type<char>::mpi_type(),
        ...
        cpy_params.srcPtr            = make_cudaPitchedPtr(
            complex, input_dim.size_y[pidx_j] * sizeof( C_t ), input_dim.size_y[pidx_j], input_dim.size_x[pidx_i]
        );
        ...
        cpy_params.extent = make_cudaExtent(
            input_dim.size_y[pidx_j] * sizeof( C_t ), transposed_dim.size_x[pidx_i], transposed_dim.size_z[pidx_j]
        );
        ...
        for ( std::size_t i = 0; i < comm_order1_.size(); i++ )
        {
            ...
            std::cout << "myid  = " << mpi_.myid << ", flag_recv: " << flag_recv << ", flag_send: " << flag_send
                      << std::endl;
        }
    }
...
private:
    ...
    struct packet_bucket
    {
        ...
        packet_bucket( const std::size_t &loc_size_p )
            : loc_size( loc_size_p ), data_buf( std::make_unique<buf_array_type>() )
        {
        ...
        {
            return data_buf->total_size() *
                   sizeof( char ); // =) char is guaranteed to be 1 byte, but 1 byte can be anythng.
        }
